local Release = "Version 1.2.106"
local NotificationDuration = 6.5
local RayfieldFolder = "Rayfield"
local ConfigurationFolder = RayfieldFolder.."/Configurations"
local ConfigurationExtension = ".rfld"

local function checktabbink(String,Start)
	...
	function Window:CreateTab(Name,Image)
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				InputSettings.CurrentValue = tostring(Input.InputFrame.InputBox.Text)
				SaveConfiguration()
			end)
			...
			function InputSettings:Set(NewVal)
				local Success, Response = pcall(function()
					InputSettings.Callback(NewVal)
				end)
				if not Success then print("InputSetting Error") end
				Input.InputFrame.InputBox.Text = tostring(NewVal)
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			for _, Option in ipairs(DropdownSettings.Options) do
				...
				DropdownOption.Interact.ZIndex = 50
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					--if Dropdown.Selected.Text ~= "" then
					if checktabbink(Option,"-") then return end
						...
						for _, droption in ipairs(Dropdown.List:GetChildren()) do
							if droption.ClassName == "Frame" and droption.Name ~= "Placeholder" and droption.Name ~= DropdownSettings.CurrentOption and not checktabbink(droption.Name,"-") then
		...
		function Tab:CreateSlider(SliderSettings)
			...
			Slider.Main.Progress.Size =	UDim2.new(0, Slider.Main.AbsoluteSize.X * ((SliderSettings.CurrentValue + SliderSettings.Range[1]) / (SliderSettings.Range[2] - SliderSettings.Range[1])) > 5 and Slider.Main.AbsoluteSize.X * (SliderSettings.CurrentValue / SliderSettings.Range[2]) or 5, 1, 0)
